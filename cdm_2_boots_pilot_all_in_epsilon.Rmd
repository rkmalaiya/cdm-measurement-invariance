...
df.error.plot <- df.error.ec.plot %>% union(df.error.ti.plot) %>% union(df.error.ss.plot) %>%
  ...
  mutate(sampling_error = sampling_error / sqrt(1000)) %>%# To get Simulation Error
...
df.t <- df.t  %>% select(parameter, group, questions, sampling_mean, avg_attempts, dataset) %>% 
  ...
  spread(key="group", value = "sampling_error") %>% rename("Partition1_error" = `Partition 1`, "Partition2_error" = `Partition 2`)
                                                                        ...
                                                                        ( sqrt( ( (Partition1_error^2)/1000 ) + ((Partition2_error^2)/1000) + 0.01^2) 
                                                                          )  
...
df.t %>% arrange(dataset, Z_test, questions, avg_attempts) %>% filter(parameter == "Guess") %>% select(-Z_avg)
