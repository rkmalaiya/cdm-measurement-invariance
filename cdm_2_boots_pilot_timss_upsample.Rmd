...
```{r}
#n=dim(X)[1]
X <- X %>% sample_frac(15, replace = TRUE)
```
...
```{r}
write_rds(df.error, "..\\data\\FirstYearProject\\df_error_upsample_timss.rds")

write_rds(df.error.agg, "..\\data\\FirstYearProject\\df_error_agg_upsample_timss.rds")

write_rds(df.error.fit, "..\\data\\FirstYearProject\\df_error_fit_upsample_timss.rds")
```


```{r}
df.error <- read_rds("..\\data\\FirstYearProject\\df_error_upsample_timss.rds")

df.error.agg <- read_rds("..\\data\\FirstYearProject\\df_error_agg_upsample_timss.rds")

df.error.fit <- read_rds("..\\data\\FirstYearProject\\df_error_fit_upsample_timss.rds")
