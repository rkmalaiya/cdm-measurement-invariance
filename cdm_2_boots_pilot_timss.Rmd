...
```{r}
write_rds(df.error, "..\\data\\FirstYearProject\\df_error_timss.rds")

write_rds(df.error.agg, "..\\data\\FirstYearProject\\df_error_agg_timss.rds")

write_rds(df.error.fit, "..\\data\\FirstYearProject\\df_error_fit_timss.rds")
```


```{r}
df.error <- read_rds("..\\data\\FirstYearProject\\df_error_timss.rds")

df.error.agg <- read_rds("..\\data\\FirstYearProject\\df_error_agg_timss.rds")

df.error.fit <- read_rds("..\\data\\FirstYearProject\\df_error_fit_timss.rds")
...
```{r}
df.fit.plot <- df.error.fit  %>% select(parameter, group, sampling_error, sample_size, attempts) %>% arrange(attempts, sample_size) %>% 
...
```{r}
df.fit.plot  %>% select(parameter,group,sample_size,attempts, sampling_error_3) %>%
  ...
  labs() +
  #facet_wrap(.~attempts, scales = "free") + 
  ggtitle("Sampling Error of Each Item")
