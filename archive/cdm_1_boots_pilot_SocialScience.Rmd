---
title: "CDM Pilot Study for Social Science (All Population)"
...
```{r}
percent_na <- round((sum(is.na(X)) / (dim(X)[1] * dim(X)[2])) * 100,1)
percent_na
```
...
df.t <- get_mean_sample_error(X.p1, X.p2, Q, sample_sizes[1], student_question_ratios[1],1)
...
for(i in 2:length(sample_sizes)) {
  ...
  df.t <- get_mean_sample_error(X.p1, X.p2, Q, sample_sizes[i], student_question_ratios[i],1)
...
```{r}
write_rds(df.error, "C:\\Users\\rkm160630\\OneDrive - The University of Texas at Dallas\\Ritesh Folder\\PhD\\FirstYearProject\\data\\df_error_socialscience_alldata.rds")

write_rds(df.error.agg, "C:\\Users\\rkm160630\\OneDrive - The University of Texas at Dallas\\Ritesh Folder\\PhD\\FirstYearProject\\data\\df_error_agg_socialscience_alldata.rds")
```


```{r}
df.error <- read_rds("C:\\Users\\rkm160630\\OneDrive - The University of Texas at Dallas\\Ritesh Folder\\PhD\\FirstYearProject\\data\\df_error_socialscience_alldata.rds")

df.error.agg <- read_rds("C:\\Users\\rkm160630\\OneDrive - The University of Texas at Dallas\\Ritesh Folder\\PhD\\FirstYearProject\\data\\df_error_agg_socialscience_alldata.rds")
...
```{r}
df.error <- df.error %>% arrange(sample_size, questions) %>% mutate(ratio = factor(ratio, levels = sort(unique(as.numeric(ratio))), ordered = TRUE)) 
df.error
```

```{r}
df.error.agg <- df.error.agg %>% arrange(sample_size, parameter, group) %>%  mutate(ratio = factor(ratio, levels = sort(unique(as.numeric(ratio))), ordered = TRUE))
df.error.agg
```
