...
df.diff %>% ggplot() + 
  aes(x=avg_attempts, y = mn, color = dataset) + 
  geom_line(aes(group = dataset)) + 
  facet_grid(.~parameter,scales = "free") + labs(x = "Average Student Attempts", y = "Mean", title = "Average of Sampling Mean Difference") +
  geom_label_repel(aes(label = round(mn,2))) 
...
ggplot() + 
  ...
  ggtitle("Effect of Student Attempts on Item Parameter's Sampling Errors") + labs(x="Average Student Attempts", y = "Typical Percentage Change (Median)") + scale_y_continuous(labels = scales::percent_format(accuracy = 2))
