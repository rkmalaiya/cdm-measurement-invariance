...
df.data %>% select(id, mastery, E1:E28) %>% gather(key = "questions", value = "answer", -id, -mastery, factor_key = TRUE) %>% group_by(mastery, questions) %>% summarize( percent_questions_answered = sum(answer) / n()) %>% 
  ggplot() + aes(x=questions, y = percent_questions_answered, fill = mastery) + geom_col(position = "dodge") + facet_grid(mastery~.) + theme_pubclean()
...
total_percent_fn <- function(df, i = 1:dim(df)[1]) {
 ...
 df.t1 <- df.t %>% select(id, mastery, E1:E28) %>% gather(key = "questions", value = "answer", -id, -mastery, factor_key = TRUE) %>% group_by(mastery, questions) %>% summarize( percent_questions_answered = sum(answer) / n()) %>% spread(key="questions", value = "percent_questions_answered") %>% ungroup() %>%select(-mastery)
...
X.bt <- boot(data = df.data , statistic = total_percent_fn, R = 1, stype = "i")
