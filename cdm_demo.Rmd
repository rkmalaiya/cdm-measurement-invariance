...
df.t <- X %>% mutate(total_marks = rowSums(.[-1]), grand_total = 28, percentage = total_marks / grand_total, 
  ...
  mutate(mastery = factor(ifelse(grades == "A" | grades == "B", "Mastery", "Non-Mastery"), levels = c("Mastery", "Non-Mastery"), ordered = TRUE)) %>%
...
df.group_non_mastery <- X.bt$t[,29:56] %>% as_tibble() %>% mutate(group = "Non-Mastery")

df.data.sim <- df.group_mastery %>% bind_rows(df.group_non_mastery)
df.data.sim
...
total_cdm_fn <- function(df, i = 1:dim(df)[1]) {
  ...
  df.t3 <- tibble("value" = df.cdm$slip$est, "key" = paste0("non_mastery_slip_E", seq(1,28,1))) %>% spread(key = "key", value = "value") %>% select(!!!paste0("non_mastery_slip_E", seq(1,28,1)))
 
  
  df.t4 <- tibble("value" = df.cdm$guess$est, "key" = paste0("non_mastery_guess_E", seq(1,28,1))) %>% spread(key = "key", value = "value") %>% select(!!!paste0("non_mastery_guess_E", seq(1,28,1)))
...
df.nm_slip <- X.bt$t[,57:84] %>%  as_tibble() %>% mutate(parameter = "Slip", group = "Non-Mastery")
df.nm_guess <- X.bt$t[,85:112] %>% as_tibble() %>% mutate(parameter = "Guess", group = "Non-Mastery")

df.data.sim <- df.m_slip %>% bind_rows(df.m_guess) %>% bind_rows(df.nm_slip) %>% bind_rows(df.nm_guess)
df.data.sim
