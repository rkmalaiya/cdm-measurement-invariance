...
```{r}
df.t <- df.data.sim
col_names <- colnames(df.t %>% select(-group)) 

df.t %>% gather(key = "questions", value="percent_questions_answered", -group) %>% mutate(questions = factor(questions, levels = col_names, ordered = TRUE)) %>% ggplot() + aes(x=questions, y = percent_questions_answered, color = group) + facet_grid(group ~ .) + geom_boxplot() 
```
