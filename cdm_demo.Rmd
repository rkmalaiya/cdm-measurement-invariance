...
total_cdm_fn <- function(df, i = 1:dim(df)[1]) {
  ...
  df.cdm <- CDM::din(df.t.mastery, Q)
 
  df.t <- df.t %>% select(id, mastery, E1:E28) %>% gather(key = "questions", value = "answer", -id, -mastery, factor_key = TRUE) %>% group_by(mastery, questions) %>% summarize( percent_questions_answered = sum(answer) / n()) %>% unite("questions",mastery, questions) %>% ungroup()
 
 
  df.t1 <- tibble("value" = df.cdm$slip$est, "key" = paste0("mastery_slip_E", seq(1,28,1))) %>% spread(key = "key", value = "value")
 
  
  df.t2 <- tibble("value" = df.cdm$guess$est, "key" = paste0("mastery_guess_E", seq(1,28,1))) %>% spread(key = "key", value = "value")
...
X.bt <- boot(data = df.data , statistic = total_cdm_fn, R = 10, stype = "i")
...
df.group_mastery <- X.bt$t %>%  %>% mutate(group = "Mastery")
df.group_non_mastery <- X.bt$t[,29:56] %>% as_tibble() %>% mutate(group = "Non-Mastery")
