...
```{r}
df.bagged.ec <- read_rds(paste0("..\\data\\FirstYearProject\\",path,"\\df_error_ecpe.rds"))

df.bagged.ec.agg <- read_rds(paste0("..\\data\\FirstYearProject\\",path,"\\df_error_agg_ecpe.rds")) #Bagged-AB

df.bagged.ec.fit <- read_rds(paste0("..\\data\\FirstYearProject\\",path,"\\df_error_fit_ecpe.rds")) %>% mutate(dataset = "ECPE")

df.sim.ec.item <- read_rds(paste0("..\\data\\FirstYearProject\\",path,"\\df_data_sim_item_ecpe.rds")) #Bootstrap-AB
...
```{r}
df.bagged.ti <- read_rds(paste0("..\\data\\FirstYearProject\\",path,"\\df_error_timss.rds"))

df.bagged.ti.agg <- read_rds(paste0("..\\data\\FirstYearProject\\",path,"\\df_error_agg_timss.rds"))

df.bagged.ti.fit <- read_rds(paste0("..\\data\\FirstYearProject\\",path,"\\df_error_fit_timss.rds")) %>% mutate(dataset = "TIMSS")
...
```{r}
df.bagged.ss <- read_rds(paste0("..\\data\\FirstYearProject\\",path,"\\df_error_socialscience.rds"))

df.bagged.ss.agg <- read_rds(paste0("..\\data\\FirstYearProject\\",path,"\\df_error_agg_socialscience.rds"))

df.bagged.ss.fit <- read_rds(paste0("..\\data\\FirstYearProject\\",path,"\\df_error_fit_socialscience.rds")) %>% mutate(dataset = "Social Psychology")
...
df.ind_z.ti %>% mutate(dataset="TIMSS") %>% 
 ...
 ggplot() + 
  ...
  labs(x="Item Administered Count (IAC)", y = "Absolute Bias", fill = "", color = "") + 
...
```{r}
 ...
 ggsave("ecpe_mab_bmab.png", dpi = "retina", width = 8, height = 3.5, units = "in")
...
df.ind_z.ti %>% select(parameter, attempts_disc, MAB) %>% rename(metric = MAB) %>% mutate(type = "Bootstrap-AB") %>% bind_rows(
 ...
 ggsave("timss_mab_bmab.png", dpi = "retina", width = 8, height = 3.5, units = "in")
...
df.ind_z.ss %>% select(parameter, MAB, attempts_disc) %>% rename(metric = MAB) %>% mutate(metric_type = "Bootstrap-AB")
 ...
 ggsave("combined_ss.png", dpi = "retina", width = 8, height = 3.5, units = "in") 
