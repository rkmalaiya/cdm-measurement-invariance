...
```{r}
write_rds(df.error, "..\\data\\FirstYearProject\\df_error_socialscience.rds")

write_rds(df.error.agg, "..\\data\\FirstYearProject\\df_error_agg_socialscience.rds")

write_rds(df.error.fit, "..\\data\\FirstYearProject\\df_error_fit_socialscience.rds")

```


```{r}
df.error <- read_rds("..\\data\\FirstYearProject\\df_error_socialscience.rds")

df.error.agg <- read_rds("..\\data\\FirstYearProject\\df_error_agg_socialscience.rds")

df.error.fit <- read_rds("..\\data\\FirstYearProject\\df_error_fit_socialscience.rds")
```
...
df.fit.plot <- df.error.fit  %>% select(parameter, group, sampling_error, sample_size, attempts) %>% arrange(attempts, sample_size) %>% 
  mutate(sampling_error_3= round(sampling_error,3),
         attempts_bin = cut(attempts, breaks = c(0,10,20,30,40,50,100, max(attempts)), right = TRUE, ordered_result = TRUE)
         )
