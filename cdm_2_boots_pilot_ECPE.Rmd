...
write_rds(df.data.sim.item, "..\\data\\FirstYearProject\\df_data_sim_item_ecpe.rds")
...
```{r}
df.fit.plot <- df.error.fit  %>% select(parameter, fit, group, sampling_mean, sampling_error, sample_size, attempts) %>% arrange(attempts, sample_size) %>% 
  mutate(sampling_error_3= round(sampling_error,3),
         attempts_bin = cut(round(attempts), breaks = 60, right = TRUE, ordered_result = TRUE)
         )
...
df.fit.plot %>% 
  ...
  ggplot() + 
  aes(x=attempts_bin, y=sampling_mean) + 
  geom_point() + 
