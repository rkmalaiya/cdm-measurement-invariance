...
total_cdm_fn <- function(df, i = 1:dim(df)[1]) {
  ...
  df.t.s1 <- df.t %>% select(E1:E28) %>% sample_frac(size = 0.5)
  df.t.s2 <- df.t %>% select(E1:E28) %>% anti_join(df.t.s1)
  ...
  df.cdm <- CDM::din(df.t.s2, Q, progress=FALSE)
 
   df.t3 <- tibble("value" = df.cdm$slip$est, "key" = paste0("s2_slip_E", seq(1,28,1))) %>% spread(key = "key", value = "value") %>% select(!!!paste0("s2_slip_E", seq(1,28,1)))
 
  
  df.t4 <- tibble("value" = df.cdm$guess$est, "key" = paste0("s2_guess_E", seq(1,28,1))) %>% spread(key = "key", value = "value") %>% select(!!!paste0("s2_guess_E", seq(1,28,1)))
  
  
  df.t <- cbind(df.t1, df.t2, df.t3, df.t4)
...
df.s1_slip <- X.bt$t[,1:28] %>%  as_tibble() %>% mutate(parameter = "Slip", group = "Partition 1")
df.s1_guess <- X.bt$t[,29:56] %>% as_tibble() %>% mutate(parameter = "Guess", group = "Partition 1")


df.s2_slip <- X.bt$t[,57:84] %>%  as_tibble() %>% mutate(parameter = "Slip", group = "Partition 2")
df.s2_guess <- X.bt$t[,85:112] %>% as_tibble() %>% mutate(parameter = "Guess", group = "Partition 2")
...
df.plot %>% filter(parameter == "Slip") %>% select(-sampling_error, -parameter) %>% spread(key = group, value = "sampling_mean") %>%
  
  ggplot() + aes(x=`Partition 1`, y = `Partition 2`) + geom_text(aes(label = questions)) + geom_abline(aes(intercept = 0, slope = 1))
...
df.plot %>% filter(parameter == "Guess") %>% select(-sampling_error, -parameter) %>% spread(key = group, value = "sampling_mean") %>%
